              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('Uskunalar topilmadi', style: TextStyle(color: AppTheme.textMuted)),
                    ],
                  ),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (_, i) => _EquipmentCard(
                    equipment: items[i],
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => EquipmentDetailScreen(id: items[i].id))),
                  ),
                ),
          loading: () => const Center(child: CircularProgressIndicator()),
          error: (e, s) => Center(child: Text('Xato: $e', style: const TextStyle(color: AppTheme.danger))),
        ),
      ),
    );
  }
...
class _EquipmentCard extends StatelessWidget {
  ...
  Color _parseStatusColor() {
    ...
    try {
      final hex = equipment.statusColor!.replaceFirst('#', '');
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: const Icon(Icons.computer_rounded, color: AppTheme.primary, size: 22),
            ),
            ...
            Container(
              ...
              child: Text(equipment.statusName ?? '—', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: statusColor)),
            ),
...
class _DetailBody extends StatelessWidget {
  ...
  Color _parseColor() {
    if (equipment.statusColor == null) return AppTheme.textMuted;
    try {
      return Color(int.parse('FF${equipment.statusColor!.replaceFirst('#', '')}', radix: 16));
    } catch (_) {
      return AppTheme.textMuted;
    }
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(equipment.statusName ?? '—', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
                      ),
            ...
            delegate: SliverChildListDelegate([
              ...
              _InfoCard(children: [
                ...
                _InfoRow(
                  ...
                  label: 'Narxi',
                  value: equipment.purchasePrice != null ? '${equipment.purchasePrice!.toStringAsFixed(0)} so\'m' : null,
                ),
              ]),
              const SizedBox(height: 16),
              ...
              if (equipment.notes != null && equipment.notes!.isNotEmpty) ...[
                ...
                Container(
                  ...
                  child: Text(equipment.notes!, style: const TextStyle(fontSize: 13, color: AppTheme.textSecondary, height: 1.5)),
                ),
              ],
                ...
                child: Column(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.qr_code_2_rounded, size: 80, color: AppTheme.textSecondary),
                    ),
                    const SizedBox(height: 12),
                    SizedBox(
                      width: double.infinity,
                          ...
                          try {
                            ...
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('QR-kod yaratildi ✅'), backgroundColor: AppTheme.success),
                              );
                            }
                          } catch (_) {}
                        },
